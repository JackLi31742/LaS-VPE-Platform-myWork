...
//                                    dataInfo.setStartTime(startTime);
                                    ...
                                    Tracklet[] tracklets =null;
                                    try {
                                    	tracklets = new RobustExecutor<Void, Tracklet[]>(
                                    			(Function0<Tracklet[]>) () -> {
										...
										// TODO: handle exception
										logger.debug("处理视频出现异常："+videoName);
										e.printStackTrace();
									}
                                    ...
                                    if (tracklets!=null&&tracklets.length>0) {
                                    logger.debug("视频"+videoName+"发送的tracklets的大小是："+tracklets.length);
                                    dataInfo.setTrackletSize(tracklets.length);
                                    	...
                                    	for (Tracklet tracklet : tracklets) {
                                    		...
                                    		final String videoRoot = metadataDir + "result/"+"ssd/" +dateString+"/" + tracklet.id.videoID;
...
//			}
    ...
    public void addToContext() throws Exception {
    	...
    	//得到的是AM Container的pid
//    	int pid=WebToolUtils.getPID();
//    	System.out.println("PedestrianTrackingApp 的pid："+pid);
    }
