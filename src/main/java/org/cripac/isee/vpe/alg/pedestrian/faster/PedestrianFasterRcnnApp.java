...
package org.cripac.isee.vpe.alg.pedestrian.faster;
...
import org.cripac.isee.alg.pedestrian.faster.Faster;
import org.cripac.isee.alg.pedestrian.faster.ObjectFaster;
 ...
 */
public class PedestrianFasterRcnnApp extends SparkStreamingApp {
    /**
	 * 
	 */
	private static final long serialVersionUID = 869808405802996398L;
	/**
	 * 
	 */
	/**
     ...
     */
    public static final String APP_NAME = "pedestrian-faster";
     ...
     */
    public PedestrianFasterRcnnApp(AppPropertyCenter propCenter) throws Exception {
        super(propCenter, APP_NAME);

        registerStreams(Collections.singletonList(new PedestrianFasterStream(propCenter)));
    }
    ...
    public static class AppPropertyCenter extends SystemPropertyCenter {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -4443652842489280730L;
    ...
    public static void main(String[] args) throws Exception {
        ...
        SparkStreamingApp app = new PedestrianFasterRcnnApp(propCenter);
        app.initialize();
        app.start();
        app.awaitTermination();
    }

    public static class PedestrianFasterStream extends Stream {

        /**
		 * 
		 */
		private static final long serialVersionUID = -3698640014497714299L;
		/**
		 * 
		 */
		public static final String NAME = "hdfs-video-faster";
        public static final DataType OUTPUT_TYPE = DataType.FASTER;

        /**
         * Port to input video URLs from Kafka.
         */
        public static final Port VIDEO_URL_PORT =
                new Port("hdfs-video-url-for-pedestrian-faster", DataType.URL);
        ...
        private class ConfCache extends HashMap<String, byte[]> {
			 ...
			 * 
			 */
			private static final long serialVersionUID = -5607867618540212629L;
        ...
        public PedestrianFasterStream(AppPropertyCenter propCenter) throws Exception {
...
//                    		rddGlom.collect().forEach(list->System.out.println("需要处理的视频:"+list));;
                    	...
                    	rddGlom.foreach(kvList -> {
                    		...
                    		if (kvList.size() > 0) {
                            logger.info("faster Partition " + TaskContext.getPartitionId()
                                    + " got " + kvList.size()
...
//                        }
                        ...
                        synchronized (PedestrianFasterStream.class) {
                        	kvList.forEach(kv->{
...
//                                    dataInfo.setStartTime(startTime);
                                    ...
                                    logger.debug("faster Received taskID=" + taskID + ", URL=" + videoURL);
                                    ...
                                    if (confFile == null) {
                                        throw new IllegalArgumentException(
                                                "faster configuration file is not specified for this node!");
                                    }
                                    ...
                                    if (!confCacheSingleton.getInst().containsKey(confFile)) {
                                        ...
                                        if (confStream == null) {
                                            throw new IllegalArgumentException(
                                                    "faster  configuration file not found in JAR!");
                                        }
                                    ...
                                    File deteFile=getResource("/models/faster/VGGnet_fast_rcnn_iter_70000.ckpt");
//                                    Detector detector = new ObjectDetection(-1,deteFile, logger);
                                    Faster faster=new ObjectFaster(-1, deteFile, logger);
...
//                                    ).execute();
                                    ...
                                    logger.debug("Finished faster  on " + videoName);
...
//                                    logger.error("During tracking.", e);
                                	System.out.println("在faster 时发生异常");
                                    e.printStackTrace();
                                }finally {
                                	try {
                                		if (hdfs!=null) {
