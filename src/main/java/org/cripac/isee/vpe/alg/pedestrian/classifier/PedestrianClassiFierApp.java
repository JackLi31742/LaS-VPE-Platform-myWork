...
package org.cripac.isee.vpe.alg.pedestrian.classifier;
...
import org.cripac.isee.alg.pedestrian.classifier.Classifier;
import org.cripac.isee.alg.pedestrian.classifier.ImgClassification;
 ...
 */
public class PedestrianClassiFierApp extends SparkStreamingApp {
    
	/**
	 * 
	 */
	private static final long serialVersionUID = -5553645761476633083L;
	/**
     * The name of this application.
     */
    public static final String APP_NAME = "pedestrian-classifier";
     ...
     */
    public PedestrianClassiFierApp(AppPropertyCenter propCenter) throws Exception {
        super(propCenter, APP_NAME);

        registerStreams(Collections.singletonList(new PedestrianClassiFierStream(propCenter)));
    }
    ...
    public static class AppPropertyCenter extends SystemPropertyCenter {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -7779620543521433235L;
    ...
    public static void main(String[] args) throws Exception {
        ...
        SparkStreamingApp app = new PedestrianClassiFierApp(propCenter);
        app.initialize();
        app.start();
        app.awaitTermination();
    }

    public static class PedestrianClassiFierStream extends Stream {

        
		/**
		 * 
		 */
		private static final long serialVersionUID = -7188490801136672994L;
		public static final String NAME = "hdfs-video-classifier";
        public static final DataType OUTPUT_TYPE = DataType.CLASSIFIER;

        /**
         * Port to input video URLs from Kafka.
         */
        public static final Port VIDEO_URL_PORT =
                new Port("hdfs-video-url-for-pedestrian-classifier", DataType.URL);
        ...
        private class ConfCache extends HashMap<String, byte[]> {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 4213564599474664259L;
        ...
        public PedestrianClassiFierStream(AppPropertyCenter propCenter) throws Exception {
...
//                    		rddGlom.collect().forEach(list->System.out.println("需要处理的视频:"+list));;
                    	...
                    	rddGlom.foreach(kvList -> {
                    		...
                    		if (kvList.size() > 0) {
                            logger.info("classifier Partition " + TaskContext.getPartitionId()
                                    + " got " + kvList.size()
...
//                        }
                        ...
                        synchronized (PedestrianClassiFierStream.class) {
                        	kvList.forEach(kv->{
...
//                                    dataInfo.setStartTime(startTime);
                                    ...
                                    logger.debug("classifier Received taskID=" + taskID + ", URL=" + videoURL);
                                    ...
                                    if (confFile == null) {
                                        throw new IllegalArgumentException(
                                                "classifier configuration file is not specified for this node!");
                                    }
                                    ...
                                    if (!confCacheSingleton.getInst().containsKey(confFile)) {
                                        ...
                                        if (confStream == null) {
                                            throw new IllegalArgumentException(
                                                    "classifier  configuration file not found in JAR!");
                                        }
                                    ...
                                    File classifierFile=getResource("/models/ClassiFier/squeezenet1_1-f364aa15.pth");
                                    File lableFile=getResource("/models/ClassiFier/labels.json");
                                    Classifier classifier =new ImgClassification(classifierFile,lableFile,logger);
...
//                                    }
                                    ...
                                    logger.debug("Performing classifier  on " + videoName);
...
//		                                    	detector.detection(videoStream);
		                                    	classifier.classifier(videoStream);
...
//                                    ).execute();
                                    ...
                                    logger.debug("Finished classifier  on " + videoName);
...
//                                    logger.error("During tracking.", e);
                                	System.out.println("在classifier 时发生异常");
                                    e.printStackTrace();
                                }finally {
                                	try {
                                		if (hdfs!=null) {
