...
public interface DeepMAR extends Recognizer {
    ...
    float MEAN_PIXEL_B = 104.0f;
    float MEAN_PIXEL_G = 117.0f;
    float MEAN_PIXEL_R = 123.0f;
    float REG_COEFF    = 1.0f;
    ...
    float[] preprocess(@Nonnull Tracklet.BoundingBox bbox) {
        ...
        */
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (i == 0) {
                sub32f(fp, 4, POINTERS.pMean32f_B, 0, fp, 4, 1, numPixelPerChannel, null);
            } else if (i == 1) {
                sub32f(fp, 4, POINTERS.pMean32f_G, 0, fp, 4, 1, numPixelPerChannel, null);
            } else if (i == 2) {
                sub32f(fp, 4, POINTERS.pMean32f_R, 0, fp, 4, 1, numPixelPerChannel, null);
            } else {
                return null;
            }
