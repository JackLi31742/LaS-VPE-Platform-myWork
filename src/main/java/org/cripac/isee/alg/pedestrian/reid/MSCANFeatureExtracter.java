...
public class MSCANFeatureExtracter implements ReIDFeatureExtracter {
    ...
    static {
        org.apache.log4j.Logger logger = 
            org.apache.log4j.Logger.getLogger(MSCANFeatureExtracter.class);
...
//        params.gpuIndex = selectGPURandomly(gpu);
        params.gpuIndex = Integer.parseInt(gpu);
        this.logger.debug("reid 使用的GPU index: " + params.gpuIndex);

        // Initialize.
        long start = System.currentTimeMillis();
        handle = initialize(params);
        long end = System.currentTimeMillis();
        this.logger.debug("MSCAN reid is initialized!，handle的值是："+handle+",使用的gpu是:"+gpu+"耗时："+(end-start)/1000+"s");
    }
     ...
     */
    @Override
    public Feature extract(@Nonnull PedestrianInfo person) throws Exception {
//        synchronized (this) {
